// 0/1 Knapsack 2D : 		[Do it Again From the course] 
...
int solveUsingRecursion(int capacity, int wt[], int profit[], int index, int n) {
	...
	/* OR
	if(index == n-1){
		if(wt[index] <= capacity) return profit[index];
		else return 0;
	}
	*/
...
void solveUsingTabulation(int capacity, int wt[], int profit[], int n) {
	...
	cout <<endl<< "Max Profit : " << dp[capacity][0] << endl;
...
//int index = 0;
	...
	solveUsingTabulation(capacity,wt, profit, n);
